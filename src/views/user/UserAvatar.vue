<script setup>
import { useTemplateRef, ref } from 'vue'
import { Plus, Upload } from '@element-plus/icons-vue'
import { useUserStore } from '@/stores'
import { userUpdateAvatarService } from '@/api/user'

const userStore = useUserStore()
// console.log(userStore.user_pic)
const imgUrl = ref(userStore.userInfo.user_pic)
// console.log(userStore)

const uploadRef = useTemplateRef('uploadRef')

// 点击选择图片按钮时，触发上传组件的点击事件
const selectFile = () => {
  const input = uploadRef.value.$el.querySelector('input')
  input.click()
}

const onUploadFile = (file) => {
  // console.log(file)

  //图片回显
  // userStore.userInfo.user_pic = URL.createObjectURL(file.raw)
  // imgUrl.value = userStore.userInfo.user_pic

  const reader = new FileReader()
  reader.readAsDataURL(file.raw)
  reader.onload = () => {
    imgUrl.value = reader.result
    // console.log(imgUrl.value)
  }
}

const uploadAvatar = async () => {
  await userUpdateAvatarService(imgUrl.value)
  userStore.userInfo.user_pic = imgUrl.value
}
</script>

<template>
  <page-container title="更换头像">
    <el-row>
      <el-col :span="12">
        <el-upload
          ref="uploadRef"
          class="avatar-uploader"
          :auto-upload="false"
          :show-file-list="false"
          :on-change="onUploadFile"
        >
          <img v-if="imgUrl" :src="imgUrl" class="avatar" />
          <img v-else src="@/assets/avatar.jpg" width="278" />
        </el-upload>
        <br />
        <el-button @click="selectFile" type="primary" :icon="Plus" size="large">
          选择图片
        </el-button>
        <el-button type="success" :icon="Upload" size="large" @click="uploadAvatar">
          上传头像
        </el-button>
      </el-col>
    </el-row>
  </page-container>
</template>

<style lang="scss" scoped>
.avatar-uploader {
  :deep() {
    .avatar {
      width: 278px;
      height: 278px;
      display: block;
    }
    .el-upload {
      border: 1px dashed var(--el-border-color);
      border-radius: 6px;
      cursor: pointer;
      position: relative;
      overflow: hidden;
      transition: var(--el-transition-duration-fast);
    }
    .el-upload:hover {
      border-color: var(--el-color-primary);
    }
    .el-icon.avatar-uploader-icon {
      font-size: 28px;
      color: #8c939d;
      width: 278px;
      height: 278px;
      text-align: center;
    }
  }
}
</style>
